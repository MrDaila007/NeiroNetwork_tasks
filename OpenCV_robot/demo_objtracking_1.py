...
while True:
    ...
    if success:     # если прочитали успешно
        ...
        if len(contours) != 0:  # если найден хоть один контур
            ...
            #print(contours[0])
            maxc2 = contours[5]    # находим наибольший контур
            ...
            if moments1["m00"] > 20: #  контуры с площадью меньше 20 пикселей не будут учитываться
                ...
                controlY = 2 * (cy1 - height / 2) / width ## находим отклонение найденного объекта от центра кадра и нормализуем его (приводим к диапазону [-1; 1])
            ...
            if moments2["m00"] > 20:  # контуры с площадью меньше 20 пикселей не будут учитываться
                cx2 = int(
                    moments1["m10"] / moments1["m00"])  # находим координаты центра контура (найденного объекта) по x
                cy2 = int(
                    moments1["m01"] / moments1["m00"])  # находим координаты центра контура (найденного объекта) по y
